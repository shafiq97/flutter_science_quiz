...
class SistemQuiz extends StatefulWidget {
  const SistemQuiz({super.key});

  @override
  _SistemQuizState createState() => _SistemQuizState();
}

class _SistemQuizState extends State<SistemQuiz> {
  List<dynamic> _questions = [];
  List<String> _selectedValues = [];

  Future<void> _fetchQuestions() async {
    final response =
        await http.get(Uri.parse('http://192.168.68.105:3001/api/data/sistem'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Sistem Rangka Manusia Quiz'),
      ),
      body: Container(
